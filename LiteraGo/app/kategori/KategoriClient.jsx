...
export default function CategoryPage() {
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      const [selectedLibrary, libraryRows, bookRows, wishlistRows] =
        await Promise.all([
          getSelectedLibraryDb(),
          fetchLibraries(),
          fetchBooks(),
  ...
  const baseByLibrary = useMemo(() => {
    return allBooks.filter((book) =>
      library ? book.libraryIds.includes(library.id) : true,
    );
  }, [allBooks, library]);

  const typeOptions = useMemo(() => {
    const options = [
      ...new Set(baseByLibrary.map((book) => book.type).filter(Boolean)),
  ...
  const categoryOptions = useMemo(() => {
    const source = baseByLibrary.filter((book) => {
      ...
      const bySection =
        activeSection === "Semua" ? true : book.section === activeSection;
      return byType && bySection;
    });

    return [
      ...new Set(source.map((book) => book.category).filter(Boolean)),
    ].sort((a, b) => a.localeCompare(b, "id-ID"));
  ...
  const filteredBooks = useMemo(() => {
    ...
    return baseByLibrary.filter((book) => {
      const byType = activeType === "Semua" ? true : book.type === activeType;
      const byCategory =
        activeCategory === "Semua" ? true : book.category === activeCategory;
      const bySection =
        activeSection === "Semua" ? true : book.section === activeSection;
  ...
  const emptyTitle = query.trim()
    ...
    : "Belum ada item pada filter ini";
    ...
    <PageShell>
      <section className="section-block" style={{ marginTop: 0 }}>
        ...
        <h1 className="section-title">
          Cari item berdasarkan judul, penulis, penerbit, atau tahun
        </h1>
        <p className="section-lead">
          Perpustakaan aktif:{" "}
          <strong>
            {library?.name || libraries[0]?.name || "Belum dipilih"}
          </strong>
          . Filter kategori otomatis mengikuti item yang tersedia di
          perpustakaan aktif.
        </p>
        <input
          className="input"
